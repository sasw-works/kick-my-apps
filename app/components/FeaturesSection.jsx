...
// Figma "Features" 4154:191 — Section 01 (4153:1867) + Section 02 (4153:2022).
...
// Illustrations are the Figma vector exports, used as-is (public/dark/features).

import React, { useSyncExternalStore } from "react";

const F = "/dark/features/";
const DESIGN_W = 1428;
...
// small square icon of Section 02 card 3 (fill + outline + png glyph)
function AppIcon({ inset, fillW, v, glyph, names }) {
  return (
...
function GroupIcon({ inset, group, pad, src }) {
  return (
    <Box inset={inset} clip>
      <Box inset="0" clip>
        <Box inset={group}>
...
function Headline({ title, subLines, subColor, subLh, subH, gapBelow }) {
  return (
    <div className="kf-headline" style={{ marginBottom: gapBelow }}>
      <div className="kf-pill">
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src={F + "s1-link-icon.svg"} alt="" width={14} height={15} />
...
export default function FeaturesSection() {
  ...
  return (
    <section className="kma-features" aria-label="Features">
      <style>{`
        /* Features frame 4154:191 — 1628 wide, #1f1f1f, 100px side padding around the 1428 body */
        .kma-features {
          position: relative;
          left: 50%;
        ...
        .kf-pill {
          ...
          background: #262626; border-radius: 999px; box-shadow: inset 0 0 0 1px #484848;
          font-size: 14px; line-height: 14px; color: #d8d8d8;
        }
        .kf-pill img { display: block; flex-shrink: 0; }
        .kf-pill span { display: block; width: 202px; height: 14px; white-space: nowrap; }
        .kf-h2 { margin: 0; font-size: 48px; line-height: 63px; font-weight: 400; color: #ffffff; min-height: 64.8px; }
        ...
        .kf-card { position: relative; flex-shrink: 0; overflow: hidden; background: #262626; border-radius: var(--radius-16); }
        .kf-card::after {
          content: ""; position: absolute; inset: 0; border: 1px solid #484848; border-radius: var(--radius-16); pointer-events: none;
        }
        .kf-illus { position: absolute; inset: 0; pointer-events: none; }
        .kf-card-title { font-size: 20px; line-height: 22px; font-weight: 400; color: #ffffff; }
        .kf-card-body { font-size: 16px; line-height: 24px; font-weight: 400; color: #a1a1a1; white-space: nowrap; }
                ...
                <div className="kf-illus">
                  <div className="kf-abs" style={{ left: "calc(50% + 0.48px)", top: 40, width: 600.95, height: 200, transform: "translateX(-50%)" }}>
                    <Box inset="14.37px 120.2px 0 120.19px" clip>
                      <Box inset="99.05px 20.05px 12.33px 19.62px" clip><Fill src="s1-instance-a.svg" /></Box>
                      <Box inset="11.6px 20.05px 99.78px 19.62px" clip><Fill src="s1-instance-b.svg" /></Box>
                    </Box>
                  </div>
                </div>
                <CenterText
              ...
              <div className="kf-card" style={{ width: 600, height: 402 }}>
                <div className="kf-illus">
                  <div
                    className="kf-abs"
                    style={{
                      left: 142, top: 76.2, width: 458, height: 126, background: "#303030",
                      borderRadius: "16px 0 0 16px", boxShadow: "inset 0 0 0 1px #484848",
                    }}
                  />
                  <Txt inset="24.93% 47.5% 69.6% 27.67%">Environment variable</Txt>
                  <div className="kf-abs" style={{ left: 166, top: 138.2, width: 230, height: 40, borderRadius: 8, boxShadow: "inset 0 0 0 1px #484848" }} />
                  <Txt inset="36.71% 64.22% 57.82% 31.28%" color="#606060">Key</Txt>
                  <div className="kf-abs" style={{ left: 412, top: 138.2, width: 230, height: 40, borderRadius: 8, boxShadow: "inset 0 0 0 1px #484848" }} />
                  <Txt inset="36.71% 22.22% 57.82% 71.28%" color="#606060">Value</Txt>
                </div>
                <CenterText
                    ...
                    <Box inset="-47.5px 60.11px 0 60.09px" clip>
                      ...
                      <Box inset="156.23px 178.7px 21.97px 28.03px" clip>
                        <Box inset="0" clip>
                          ...
                          <Txt inset="20.59% 44.56% 47.66% 6.84%">/etc/config/my_config.yaml</Txt>
                          <Txt inset="49.45% 75.35% 18.8% 6.07%" size={13} color="#a1a1a1">5 hours ago</Txt>
                          <Box inset="20.88% 5.38% 21.4% 84.44%"><Fill src="s1-cfg1-avatar.svg" /></Box>
                      ...
                      <Box inset="37.64px 32.37px 140.57px 188.78px" clip>
                        <Box inset="0" clip>
                          ...
                          <div className="kf-abs" style={{ left: "84.03%", right: "5.41%", top: "calc(50% + 0.42px)", aspectRatio: "1 / 1", transform: "translateY(-50%)" }}>
                            <Fill src="s1-cfg2-avatar.svg" />
                          </div>
                          <Txt inset="34.44% 9.37% 33.81% 87.99%" color="#aeddff">S</Txt>
                          <Txt inset="20% 42.39% 48.25% 7.16%">/etc/config/my_config.yaml</Txt>
                          <Txt inset="48.87% 75.14% 19.38% 7.16%" size={13} color="#a1a1a1">2 mins ago</Txt>
                        </Box>
                      </Box>
                    </Box>
                  </div>
